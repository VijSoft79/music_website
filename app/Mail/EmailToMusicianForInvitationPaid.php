...
class EmailToMusicianForInvitationPaid extends Mailable
{
    ...
    public function envelope(): Envelope
    {
        ...
        if ($this->offer->offer_type === 'free-option') {
            ...
            $subject = "You Have Chosen a Free Option from {$this->offer->user->name}";
        } 
        elseif ($this->offer->offer_type === 'standard') {
            $this->template->offer_price == 0 ? $subject = "You Have Chosen a Free Option from {$this->offer->user->name}" : $subject;
        }
    ...
    {
        return new Content(
            view: 'emails.email-to-musician-for-invitaion-paid',
        );
    }
